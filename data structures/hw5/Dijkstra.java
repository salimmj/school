...
// msm2243
// GUI implementation of DIjkstra's algorithm
//***********************
...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
    double dist = Math.sqrt(Math.pow((ux-vx),2) + Math.pow((uy-vy),2));
    return dist;
  }
  ...
  public void computeAllEuclideanDistances() {
    ...
    for (Vertex element : getVertices()) {
        for (Edge e : element.adjacentEdges) {
            e.distance = computeEuclideanDistance(element.x, element.y, e.target.x, e.target.y);
        }
    }
  }
  ...
  public void doDijkstra(String s) {
    ...
    getVertex(s).distance = 0.0;
    getVertex(s).prev = getVertex(s);

    while (!unvisited.isEmpty()) {
      current = findMin(unvisited);
      unvisited.remove(current);
      for (Edge e : current.adjacentEdges) {
        if (unvisited.contains(e.target) && e.target.distance > e.distance + current.distance) {
          e.target.distance = e.distance + current.distance;
  ...
  private Vertex findMin(ArrayList<Vertex> l) {

    Vertex min = l.get(0);

    if (l.size() > 1) {
      for (int i = 1; i < l.size(); i++) {
        if (min.distance > l.get(i).distance)
          min = l.get(i);
      }
    }
  ...
  private boolean isConnected(Vertex v, Vertex min) {
    boolean b = false;
    for (Edge e : v.adjacentEdges) {
      if (e.target.equals(min))
        b = true;
    }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    while (!current.name.equals(s)) {

      for (Edge e : current.prev.adjacentEdges) {
        if (e.target.equals(current)) {
          l.add(e);
        }
      }
    ...
    System.out.println("Distance is going to be: " + distance);
